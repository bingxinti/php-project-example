...
class HaoResult extends AbstractModel{
    ...
    public static function init($errorCode = null,$result = null,$extraInfo = null) {
        ...
        if(Utility::isDeployStatus())
        {
            if(empty($_REQUEST))
            {
                $_REQUEST = array_merge($_GET,$_POST);
            }
            $path                           = sprintf("%s",$_SERVER['REQUEST_URI']);
            $_REQUEST['api']                = $path;
            $_REQUEST['file']                = $_FILES;
            // $extraInfo['request_method'] = $_SERVER['REQUEST_METHOD'];
            // $_REQUEST['host']            = $_SERVER['HTTP_HOST'];
            $extraInfo['debug']             = $_REQUEST;
            // $extraInfo['getallheaders']      = getallheaders();
        }
    ...
    /**
     * [initError 初始化一个错误]
     * @param  string $errorStr  [description]
     * @return [type]            [description]
     */
    public static function initError($errorStr = '未指定错误类型', $errorCode =  null)
    {
        $errorCode = is_null($errorCode) ? ERROR_CODE::$ERROR : $errorCode ;
        $haoResult =  HaoResult::init($errorCode);
        $haoResult->setErrorStr($errorStr);
        return $haoResult;
    }
