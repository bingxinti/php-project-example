// http://test-wechat.dawennet.com/web/api/webroot/index.php?r=axapi/check_captcha
var currentUrl;
...
function range_to_badge(range) {
    var s = '';
    if (range < 0) {} else if (range < 60 * 60) {
...
function reFormApi(i) {
    var _api = apiList[i];
    console.log('_api1',_api,i,apiList);
...
function apiKeyBarInit() {
    ...
    for (var i in _keyTypeArray) {
        ...
        $('#switch_examples').append('<button type="button" class="btn btn-default" onclick="changeKeyType(this);" keytype="' + _keyType + '" unique-id="' + hex_md5(_keyType) + '" >' + _keyType + (_sortType == 'time' ? range_to_badge(_now - _timeunix) : (_lastKeyPY != _keyPY ? '<span class="badge">' + _keyPY + '</span>' : '')) + '</button>');
        _lastKeyPY = _keyPY;
...
function apiListInit() {
    ...
    for (var i in apiList) {
        ...
        if (_api['remind'] && _api['remind'] != '')
        {
            _api['title'] = _api['title'] + '<span class="user-remind">' + _api['remind'] + '</span>'
        }
        if (_api['desc'] != '') {
            // _api['desc'] = _api['desc'] ? '<span class="user-desc">' + _api['desc'] + '</span>' : _api['desc'];
    ...
    <a id="btn_api_title_' + i + '" href="javascript:;" data-toggle="collapse" data-parent="#list_api_btns" data-target="#collapseDiv' + i + '" onclick="reFormApi(' + i + ');" style="width: 100%;display: inline-block;">' + (range_to_badge(_now - _api['timeunix'])) + _api['title'] + '<span class="span-method">' + _api['method'] + '</span>' + '</a>' + '\
    </h4>\
    </div>\
