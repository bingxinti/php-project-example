  ...
  if(is_dir($dir))
  {
    if ($dh = opendir($dir))
    {
      while (($file = readdir($dh)) !== false)
      {
        if($file!="." && $file!="..")
        {
            $path = "{$dir}/{$file}/{$file}Api.js";
            if(file_exists($path))
            {
              $script[] = "{$file}/{$file}Api.js";
              // $script[] = $path;
            }
        }
      }
...
$arrScript = loadListScript('../../mhc');
foreach ($arrScript as $fileScript)
{
    echo "\n<script type='text/javascript' src='script.php?path={$fileScript}' ></script>";
  // echo file_get_contents($fileScript);
}
