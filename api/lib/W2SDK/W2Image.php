...
class W2Image {
    ...
    public static function getImgSource($img)
    {
        $source = null;
        if(!is_resource($img))
        {
            ...
            {
                $imgInfo = W2Web::loadStringByUrl($img,'get',null,null,60,'all');
                $isImage = !empty($imgInfo['header']) && strpos($imgInfo['header'],'image')!==false && !empty($imgInfo['body']);
                // 检查图片资源是否正常图片
                    // HTTP/1.1 200 OK Date: Fri, 12 Jan 2018 12:12:35 GMT Server: Apache/2.4.25 (Unix) PHP/5.5.38 Last-Modified: Wed, 27 Dec 2017 04:04:42 GMT ETag: "6591-5614a7fc7d79f"Accept-Ranges: bytes Content-Length: 26001 Content-Type: image/jpeg
            ...
            if(!empty($imgTypeInt))
            {
                ...
                // D($img);
                // D( exif_imagetype($img) ) ;
                // D( read_exif_data($img) ) ;
                //
                // 1 IMAGETYPE_GIF
                // 2 IMAGETYPE_JPEG
                // 3 IMAGETYPE_PNG
                ...
                // 15 IMAGETYPE_WBMP
                // 16 IMAGETYPE_XBM
                // $img = 'http://d.lanrentuku.com/down/png/1712/if_christmass_holidays_celebrate/christmass_star_2.png';
                // D( pathinfo($img) ) ;
            }
            switch($ext){
                case 'png':
                ...
                default:
                // $imgInfo = W2Web::loadStringByUrl($img,'get',null,null,60,'all');
                // if(!empty($imgInfo['header']) && strpos($imgInfo['header'],'image')!==false )
                // {
                //     $source = $imgInfo['body'];
                //     if(!empty($source))
                //     {
     ...
     * @param int    压缩质量
     */
    public static function makePhotoThumb($srcFile,$photo_small,$dstW,$dstH,$p_quality=100) {
    $data = @getimagesize($srcFile);
