...
// 订阅号
if(!defined('SUBSCRIBE_TOKEN'))
{
	define('SUBSCRIBE_TOKEN', '');
	define('SUBSCRIBE_ENCODINGAESKEY', '');
	define('SUBSCRIBE_APPID', '');
	define('SUBSCRIBE_APPSECRET', '');
}
...
{
	define('SERVER_TOKEN', '');
	define('SERVER_ENCODINGAESKEY', '');
	define('SERVER_APPID', '');
	define('SERVER_APPSECRET', '');
}
...
class W2Wechat extends Wechat
{
	 ...
	 */
    public final static function getInstance($wxType = SELF::SERVER_TYPE)
    {
        if(is_null(self::$instance))
        {
            self::$instance = new static($wxType);
        }
	 ...
	 */
	public final function __construct($wxType)
	{
		$options = array();
		// 订阅号
		if($wxType == SELF::SUBSCRIBE_TYPE)
		{
			$options['token'] = SUBSCRIBE_TOKEN;
			$options['encodingaeskey'] = SUBSCRIBE_ENCODINGAESKEY;
			$options['appid'] = SUBSCRIBE_APPID;
			$options['appsecret'] = SUBSCRIBE_APPSECRET;
		}
		// 服务号
		else if($wxType == SELF::SERVER_TYPE)
		{
			$options['token'] = SERVER_TOKEN;
			$options['encodingaeskey'] = SERVER_ENCODINGAESKEY;
			$options['appid'] = SERVER_APPID;
			$options['appsecret'] = SERVER_APPSECRET;
		}
...
// $weObj = ZWechat::getInstance();
// $type = $weObj->getRev()->getRevType();
// D($type);
// ZFile::filePut('./params.txt',$type,1);
// switch($type)
// {
// 	case Wechat::MSGTYPE_TEXT:
// 		$weObj->text("hello, I'm wechat")->reply();
...
// 'token'          =>'chetupinche', //填写你设定的key
// 			'encodingaeskey' =>'cZvE2D1nRz0tcGapTNBPJcOh2UA5nZxabX1T4F0c0l4', //填写加密用的EncodingAESKey
// 			'appid'          =>'wx45ccb3fbcf963f59', //填写高级调用功能的app id, 请在微信开发模式后台查询
// 			'appsecret'      =>'50150ef27c3016a333918c27e2b1fdaa' //填写高级调用功能的密钥

/*
$options = array(
        'token'=>'chetupinche',   //填写应用接口的Token
        'encodingaeskey'=>'cZvE2D1nRz0tcGapTNBPJcOh2UA5nZxabX1T4F0c0l4',//填写加密用的EncodingAESKey
        'appid'=>'wx45ccb3fbcf963f59',  //填写高级调用功能的appid
        'appsecret'=>'50150ef27c3016a333918c27e2b1fdaa',  //填写高级调用功能的appid
);
$weObj = new Wechat($options);
