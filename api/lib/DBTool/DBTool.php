...
class DBTool
{
	...
	public static function queryData($p_sqls, $p_encode="utf8") {
	    ...
	    if (count($_sqls)>0 ) {
			...
			foreach ($_sqls as $_sql) {
				$_tmpData = array();
		        $_resultSet = static::mysqli_query_sql($_sql);
		        if($_resultSet !== false){
    ...
    /**
     * [putLogSql 记录执行的SQL]
     * @param  integer $executeTime [description]
     * @param  string  $sql         [description]
     * @return [type]               [description]
     */
    public static function putLogSql($executeTime = 0, $sql = '')
    {
    	$userID = Utility::getHeaderValue('Userid');
    	$userID = intval($userID);

		$executeTime = number_format($executeTime, 5, '.', '');
		$logSql      = "[{$executeTime}] {$sql}";

		/** 记录操作数据的sql语句 */
		if(preg_match('/(^insert)|(^update)|(^delete)|(^alter)|(^create)|(^drop)|(^grant)|(^truncate)|(^call)/i', trim($sql) ))
		{
			file_put_log($logSql,'execute_sql', $userID);
		}
		// 慢查询记录
		if($executeTime > 2)
		{
			file_put_log($logSql,'slow_sql', $userID);
		}

    }

    /** [_query_sql 所有的SQL都走这里执行的mysql] */
	public static function mysqli_query_sql($_sql)
	{
		self::debug($_sql);
		$_mysqli      = self::getCoon();
        ...
        static::putLogSql($sqlTimeEnd-$sqlTimeBegin,$_sql);
		return $result;
	}


    /**
	 * [$transactions 采用计数的方式管理嵌套事务]
	 * 事务首次开启计数+1, 再次开启只+1不开启，回滚等于1时才提交回滚
	 * @var integer
	 */
	 ...
	 * 事务提交
	 */
	private static function _transCommit()
	{
		if(self::$transactions == 1)
		{
			static::mysqli_query_sql('COMMIT');
		}
		--self::$transactions;
	}
	/**
	 * 事务回滚
	 */
	private static function _transRollback()
	{
		if(self::$transactions == 1)
		{
			static::mysqli_query_sql('ROLLBACK');
		}
		--self::$transactions;
	}
	/**
	 * 事务结束
	 */
	private static function _transEnd()
	{
		if(self::$transactions == 0)
		{
			static::mysqli_query_sql('END');
		}
	}
}
