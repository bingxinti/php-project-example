...
class DBModel{
		...
		public function randomList($limit = 1)
		{
			// 最多500条
			$limit     = empty($limit) ? 1 : $limit;
			$maxLimit  = min(500,intval($limit));
			$tableName = $this->tableName;
			$sql       =  "SELECT t1.* FROM {$tableName} AS t1 JOIN (SELECT ROUND(RAND() * ((SELECT MAX(id) FROM {$tableName}) - (SELECT MIN(id) FROM {$tableName})) + (SELECT MIN(id) FROM {$tableName})) AS id ) AS t2 WHERE t1.id >= t2.id ORDER BY t1.id LIMIT {$maxLimit};";
			$listData  = DBTool::queryData($sql);
	        return $listData;
		}
		...
		{
			$sql = 'select column_default,column_name, column_comment,character_maximum_length,data_type from information_schema.columns where   table_schema = "'.DB_DATABASE.'" and table_name = "'. $this->tableName .'"' ;
			$_data = DBTool::queryData($sql);
			$list = array();
			foreach ($_data as $key => $value)
			{
				$list[$value['column_name']] = array
				(
	   ...
	   /** [getTables description] */
		...
		{
			$sql = 'select table_name, table_comment from information_schema.tables where table_schema = "' . DB_DATABASE .'"';
			$_data = DBTool::queryData($sql);
			$list = array();
			foreach ($_data as $key => $value)
			{
				$list[$value['table_name']] = array
				(
